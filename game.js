...
// Hardcoded changelog fallback: update manually when releasing.
const HARDCODED_CHANGELOG = `CHANGELOG
...
Added debug logging for player notifications in the connectSocket function.
...
Refactored leaderboard display to synchronize live player data.
...
Improved construction timing and animation handling for smoother visuals.
...
function showChangelog() {
    ...
    // If we have a hardcoded changelog, display it immediately (reliable offline view)
    if (typeof HARDCODED_CHANGELOG === 'string' && HARDCODED_CHANGELOG.trim().length > 0) {
        content.textContent = HARDCODED_CHANGELOG;
...
function placeOrSelect(id) {
    ...
    if (!game.selectedMode) {
        // If this is a queued ghost owned by us, open the cancel-queue popup
        const queued = game.buildings.find(b => b.id === id && b._queued && b.ownerId === getLocalPlayerId());
        if (queued) {
            showCancelQueueMenu(id);
            return;
        }

        // If it's our completed/placed building, open sell popup
...
/**
 * Show cancel-queued-build menu for player's queued ghost
 */
function showCancelQueueMenu(cellId) {
    const q = (game._buildQueue || []).find(x => x.cellId === cellId);
    if (!q) return;
    ...
    const menu = document.createElement('div');
    menu.className = 'cancel-queue-menu';
    menu.style.cssText = `position:fixed; left:${menuLeft}px; top:${menuTop}px; background:rgba(0,0,0,0.95); border:1px solid #4db8ff; border-radius:4px; padding:8px 10px; z-index:100; min-width:${menuW}px; box-shadow:0 6px 18px rgba(0,0,0,0.8);`;

    const title = document.createElement('div');
    title.style.cssText = 'font-weight:700;color:#4db8ff;margin-bottom:6px;';
    title.textContent = `Cancel queued ${displayNames[q.type] || q.type}`;
    menu.appendChild(title);

    const body = document.createElement('div');
    body.style.cssText = 'color:#ddd;font-size:13px;margin-bottom:8px;';
    body.innerHTML = `<div>Cancel this queued build and refund (partial/full) will be applied by the server.</div>`;
    menu.appendChild(body);
    ...
    const doCancelBtn = document.createElement('button');
    doCancelBtn.textContent = 'Cancel Queue';
    doCancelBtn.style.cssText = 'background:#ff6b6b;color:#fff;border:none;padding:6px 10px;border-radius:4px;cursor:pointer;font-weight:700;';
    doCancelBtn.onclick = () => {
        cancelQueuedBuild(cellId);
        menu.remove();
    };
    btnRow.appendChild(doCancelBtn);
...
function calculatePower() {
    ...
    game.buildings.forEach(building => {
        ...
        if (building.type === 'plant') {
            ...
            // road-proximity bonus: reactor next to a road tile sells power to more customers (+22%)
            const roadMult = cellHasRoadNeighbor(building.id) ? 1.22 : 1.0;
            // same-type proximity bonus: nearby reactors share grid load (+11% per neighbor, cap 3)
            let samePlants = 0;
            game.buildings.forEach(other => {
            ...
            const plantProxMult = 1 + (Math.min(samePlants, 3) * 0.11);
...
function productionTick() {
    ...
    activeMines.forEach(mine => {
        ...
        // Apply same-type proximity bonus: nearby mines share resources (+11% per neighbor, cap 3)
        let sameMines = 0;
        game.buildings.forEach(other => {
            if (other.id !== mine.id && other.type === 'mine' && !other.isUnderConstruction
                && distance(mine.id, other.id) <= game.proximityRange) sameMines++;
        });
        amount *= (1 + (Math.min(sameMines, 3) * 0.11));
    ...
    activeProcessors.forEach(processor => {
        ...
        // Apply same-type proximity bonus: co-located plants share logistics (+11% per neighbor, cap 3)
        let sameProcessors = 0;
        game.buildings.forEach(other => {
            if (other.id !== processor.id && other.type === 'processor' && !other.isUnderConstruction
                && distance(processor.id, other.id) <= game.proximityRange) sameProcessors++;
        });
        amount *= (1 + (Math.min(sameProcessors, 3) * 0.11));
        // Apply road-proximity bonus: faster material runs to reactors (+15% throughput)
        if (cellHasRoadNeighbor(processor.id)) amount *= 1.15;
        converted += amount;
    });
...
function onCellHover(id, e) {
    ...
    if (!player && !enemy && game.selectedMode && game.selectedMode !== 'sabotage') {
        ...
        if (type === 'plant') {
            ...
            content += `<div style="color:${hasRoad ? '#4CAF50' : '#888'};">` +
                `🛣️ Road access: ${hasRoad ? '+22% income if built here ✓' : 'no road bonus here'}</div>`;
        }
        if (type === 'processor') {
            const hasRoad = cellHasRoadNeighbor(id);
            content += `<div style="color:${hasRoad ? '#4CAF50' : '#888'};">` +
    ...
    if (player && !game.selectedMode) {
        ...
        if (type === 'plant') {
            const hasRoad = cellHasRoadNeighbor(id);
            content += `<div style="color:${hasRoad ? '#4CAF50' : '#888'};">` +
                `🛣️ Road access: ${hasRoad ? '+22% income ✓' : 'none'}</div>`;
        }
        if (type === 'processor') {
            const hasRoad = cellHasRoadNeighbor(id);
            content += `<div style="color:${hasRoad ? '#4CAF50' : '#888'};">` +
